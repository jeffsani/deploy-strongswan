...
%{ for i, t in tunnels ~}
  *vpn-IKEv2-${i+1}*)
    ...
    RT_TABLE="${t.rt_table}"
    VTI_MARK="${t.mark}"
    PRIORITY_TRAFFIC=$((80 + ${i}))
    PRIORITY_HEALTH=$((90 + ${i}))
    ;;
%{ endfor ~}
  *)
    VTI_IF="vti1"
...
case "$${PLUTO_VERB}" in
  up-client)
    ...
    # FIX: Explicitly direct health check responses generated by the private interface back down the VTI
    ip rule add from "$${CUSTOMER_IP}/32" lookup "$${RT_TABLE}" priority "$${PRIORITY_HEALTH}" 2>/dev/null || true

    # Dynamic Selector Mode Block
    if [ "${tunnel_flow_traffic_only}" = "true" ]; then
      ...
      ip rule add to 162.159.65.1/32 lookup "$${RT_TABLE}" priority "$${PRIORITY_TRAFFIC}" 2>/dev/null || true
    else
      echo "Routing configuration: Directing ALL transit traffic from LAN 192.168.15.0/24 to $${VTI_IF}"
      ip rule add from 192.168.15.0/24 lookup "$${RT_TABLE}" priority "$${PRIORITY_TRAFFIC}" 2>/dev/null || true
    fi
    ;;
  down-client)
    ...
    if [ "${tunnel_flow_traffic_only}" = "true" ]; then
      ip rule del to 162.159.65.1/32 lookup "$${RT_TABLE}" priority "$${PRIORITY_TRAFFIC}" 2>/dev/null || true
    else
      ip rule del from 192.168.15.0/24 lookup "$${RT_TABLE}" priority "$${PRIORITY_TRAFFIC}" 2>/dev/null || true
    fi
